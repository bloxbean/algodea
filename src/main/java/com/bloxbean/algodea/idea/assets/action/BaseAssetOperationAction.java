...
public abstract class BaseAssetOperationAction extends AlgoBaseAction {
    private final static Logger LOG = Logger.getInstance(BaseAssetOperationAction.class);

    public BaseAssetOperationAction() {
        super();
    }
...
//        AssetCacheService assetCacheService = AssetCacheService.getInstance();
        try {
            ...
            Task.Backgroundable task = new Task.Backgroundable(project, getTxnCommand()) {
                ...
                public void run(@NotNull ProgressIndicator indicator) {
                    ...
                    try {
                       // boolean status = assetTransactionService.destroyAsset(managerAccount, finalAssetTxnPrameters, txnDetailsParameters);
                        boolean status = invokeAssetOperation(assetTransactionService, senderAccount, finalAssetTxnPrameters, txnDetailsParameters);
                        if(status) {
                            console.showInfoMessage(String.format("%s was successful with asset id %s", getTxnCommand(), finalAssetTxnPrameters.assetId));
    ...
    protected abstract boolean invokeAssetOperation(AssetTransactionService assetTransactionService, Account sender, AssetTxnParameters finalAssetTxnPrameters, TxnDetailsParameters txnDetailsParameters) throws Exception;
